    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Title, v.Description, v.Url, v.DateCreated as VideoCreatedDate, v.UserProfileId, u.Name as UserName, u.Email, u.ImageUrl, u.DateCreated as UserDateCreated
                          FROM Video v
                          LEFT JOIN UserPrile on v.UserProfileId = u.Id
                          WHERE v.Id = @Id";
        ...
        public Video GetVideoByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                    SELECT v.Id AS VideoId, v.Title, v.Description, v.Url, 
                        v.DateCreated AS VideoCreatedDate, v.UserProfileId As VideoUserProfileId,

                        up.Name AS UserName, up.Email, up.DateCreated AS UserProfileDateCreated,
                        up.ImageUrl AS UserProfileImageUrl,
        ...
        public List<Video> Hottest(DateTime inputdate)
        {
            using (var conn = Connection)
            {
                conn.Open();
              ...
              SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated AS VideoDateCreated, v.UserProfileId,
               ...
               WHERE v.DateCreated >= @inputdate";
                    ...
                    cmd.CommandText = sql;
                    DbUtils.AddParameter(cmd, "@inputdate", inputdate);
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
